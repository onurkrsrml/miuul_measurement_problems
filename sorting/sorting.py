...
# değerleri standartlaştırma(yani değerleri birbirleri ile çarpma-bölme işlemi sonrası yorumlama yapmak istediğimiz zaman
# satın alma sayıları 5 haneli / yorumlar 3-4 haneli. ama rating/puanlar kısmı ise tek haneli sayılardan oluşmakta
# tam da bu sebepten standartlaştırma uygularız. tüm değişkenleri rating(1-5) değerlerine standartlaştırırız
df["purchase_count_scaled"] = MinMaxScaler(feature_range=(1, 5)). \
    fit(df[["purchase_count"]]). \
    transform(df[["purchase_count"]])
df.head(10)
# MinMaxScaler(feature_range=(1, 5)) oluşturulacak yeni değişkenin aralığı 1-5 arası
# fit(df[["purchase_count"]]) ilgili değişkenleri verilen aralıkta dönüştürmeyi yap
# transform(df[["purchase_count"]]) # işlemi tamamla, değiştirdiğin değerlere bunu dönüştür
# df["purchase_count_scaled"] bu işlemi yeni değişkene kaydet

df.describe().T # describe fonksiyonu ile değişkenlerin istatistiksel değerlerini görebiliriz. burada min ve max değerlerine bakarak
# standartlaştırma işleminin doğru yapıldığını görebiliriz.
# .T ile transpozunu alarak daha okunabilir bir hale getiririz. satır ve sütun yer değiştirir.


# üstteki fonk comment_count için de oluşturuyoruz
df["comment_count_scaled"] = MinMaxScaler(feature_range=(1, 5)). \
    fit(df[["commment_count"]]). \
    transform(df[["commment_count"]])
df.head(10)

# score: ilgili değerlerin bir araya gelerek ve ağırlaştırılarak oluşturulan score lardır.
(df["comment_count_scaled"] * 32 / 100 +
 df["purchase_count_scaled"] * 26 / 100 +
 df["rating"] * 42 / 100)

# fonk a geçirip ağırlık değerlerini kolay değiştirilebilir kılmak
...
# kurs adına göre filtrele sonra fonk uygula
...
# puan dagiliminin uzerinden agirlikli bir sekilde olasiliksal ortalama hesaplar
...
# kurslarin puanlarina bayesian average rating fonk uygula
# FeatureWarning aliyoruz. .values kullanarak cozebiliriz
...
# - Weighted Rating
# - Bayesian Average Rating Score (Eklendi)
